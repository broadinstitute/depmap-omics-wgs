...
src_samples = (
    legacy_workspace.get_entities("sample")[
        ["sample_id", "mutect2_filtered_vcf", "mutect2_filtered_vcf_idx"]
    ]
    .rename(
        columns={
            "mutect2_filtered_vcf": "mut_filtered_vcf",
            "mutect2_filtered_vcf_idx": "mut_filtered_vcf_idx",
        }
    )
    .dropna(subset=["mut_filtered_vcf"])
)
...
src_samples = src_samples.loc[
    src_samples["sample_id"].isin(
        dest_samples.loc[dest_samples["mut_filtered_vcf"].isna(), "sample_id"]
    )
]
...
for batch in batch_evenly(melted_todo.sort_index(ascending=False), 10):
    print(batch)
