...
# Adapted from https://github.com/broadinstitute/gatk/blob/4.5.0.0/scripts/cnv_wdl/somatic/cnv_somatic_pair_workflow.wdl
#
# Modifications:
#   - Remove Funcotator option and imported subworkflow
...
task DenoiseReadCounts {
    ...
    command <<<
        ...
        java -Xmx~{command_mem_mb}m -jar ${GATK_LOCAL_JAR} DenoiseReadCounts \
            --input ~{read_counts} \
...
task ModelSegments {
    ...
    command <<<
        ...
        java -Xmx~{command_mem_mb}m -jar ${GATK_LOCAL_JAR} ModelSegments \
...
task CallCopyRatioSegments {
    ...
    command <<<
        ...
        java -Xmx~{command_mem_mb}m -jar ${GATK_LOCAL_JAR} CallCopyRatioSegments \
...
task PlotDenoisedCopyRatios {
    ...
    command <<<
        ...
        java -Xmx~{command_mem_mb}m -jar ${GATK_LOCAL_JAR} PlotDenoisedCopyRatios \
...
task PlotModeledSegments {
    ...
    command <<<
        ...
        java -Xmx~{command_mem_mb}m -jar ${GATK_LOCAL_JAR} PlotModeledSegments \
            --denoised-copy-ratios ~{denoised_copy_ratios} \
